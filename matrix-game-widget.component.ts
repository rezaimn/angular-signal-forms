...
export class MatrixGameWidgetComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild('loadingScreen') loadingScreenEl: ElementRef;
  ...
  public isLoadingFadingOut: boolean = false;
  ...
  public get shouldDisplayLoadingScreen(): boolean {
  ...
  ngAfterViewInit() {
    ...
    if (this.shouldDisplayLoadingScreen) {
      this.shouldShowLoading = true;
    }

    // Watch for changes to trigger fade-out
    this.store.pipe(select(selectIntegrationMode), takeUntil(this.destroy$)).subscribe(() => {
      this.checkLoadingState();
    });
  }

  /**
   * Check if loading screen should fade out
   */
  private checkLoadingState() {
    const shouldDisplay = this.shouldDisplayLoadingScreen;
    
    // If loading should no longer be displayed and it's currently showing, trigger fade-out
    if (!shouldDisplay && this.shouldShowLoading && !this.isLoadingFadingOut) {
   ...
   */
  public onLoadingAnimationEnd() {
    if (this.isLoadingFadingOut) {
