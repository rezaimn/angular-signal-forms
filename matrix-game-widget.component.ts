...
export class MatrixGameWidgetComponent implements AfterViewInit, OnDestroy {
  ...
  private previousTemplateType: string = '';
  ...
  /**
   * Handle template changes to trigger loading animation fade-out
   */
  private handleTemplateChange(data: { type: string; payload?: any }) {
    // If we're transitioning away from Loading state, trigger fade-out animation
    if (this.previousTemplateType === 'Loading' && data.type !== 'Loading' && this.loadingAnimationElement) {
    ...
    this.previousTemplateType = data.type;
  }
