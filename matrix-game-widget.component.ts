...
import { AfterViewInit, Component, ElementRef, EventEmitter, HostListener, Input, NgZone, OnDestroy, Output, Type, ViewChild, ViewContainerRef } from '@angular/core';
...
export class MatrixGameWidgetComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild('loadingAnimationElement', { static: false }) loadingAnimationElement: ElementRef;
  ...
  constructor(
    ...
    this.store.pipe(select(selectIntegrationMode), distinctUntilKeyChanged('type'), delay(5), takeUntil(this.destroy$)).subscribe((data: { type: string; payload?: any }) => {
      this.ngZone.run(() => {
        const previousShowLoading = this.shouldShowLoadingAnimation();
        this.integrationModeTemplate = Object.assign({}, data);
        const currentShowLoading = this.shouldShowLoadingAnimation();
        
        // Handle fade-out animation when loading animation should be hidden
        if (previousShowLoading && !currentShowLoading && this.showLoadingAnimation) {
          this.fadeOutLoadingAnimation();
        } else if (!previousShowLoading && currentShowLoading) {
  ...
  private shouldShowLoadingAnimation(): boolean {
    return !this.integrationModeTemplate?.type && this.platform?.gameLoadingAnimation;
  }

  private fadeOutLoadingAnimation() {
    if (this.loadingAnimationElement?.nativeElement) {
      const element = this.loadingAnimationElement.nativeElement;
      element.classList.add('nrgs-loading-fade-out');
      ...
      // Fallback if element ref is not available
      this.showLoadingAnimation = false;
    }
  }
  ...
  ngAfterViewInit() {
    ...
    // Initialize loading animation state
    if (this.shouldShowLoadingAnimation()) {
      this.showLoadingAnimation = true;
    }
  }
