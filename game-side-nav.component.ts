...
@Component({
  ...
  imports: [GameBonusOverviewComponent, RacesNavComponent, GameSearchComponent, CommonModule, TranslateModule],
})
export class GameSideNavComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  @ViewChild('sideNavElement', { static: false }) sideNavElement: ElementRef<HTMLElement>;
  ...
  public sideNavState: SideNavState = 'closed';
  ...
  ngOnInit(): void {
    this.fullScreenService.fullscreenState$.pipe(takeUntil(this.destroy$)).subscribe(
      (fullscreenState) => {
        this.fullscreen = fullscreenState;
      },
    ...
    this.sideNavState$.pipe(takeUntil(this.destroy$)).subscribe((sidenavState: 'open' | 'closed') => {
      this.sideNavState = sidenavState;
      this.updateAnimationState(sidenavState);
  ...
  ngAfterViewInit(): void {
    ...
    this.updateAnimationState(this.sideNavState);
  }

  private updateAnimationState(state: SideNavState): void {
    ...
    if (state === 'open') {
      // Remove display: none first to allow transition
      element.style.display = '';
      // Force reflow to ensure display change is applied
      element.offsetHeight;
      // Set transition for opening animation
      element.style.transition = 'transform 400ms cubic-bezier(0.25, 1, 0.5, 1), opacity 400ms cubic-bezier(0.25, 1, 0.5, 1)';
      // Remove closed class and add open class
      element.classList.remove('closed');
      element.classList.add('open');
    } else {
      // Set transition for closing animation
  ...
  public closeNavigation(): void {
    ...
    const gameInfoPage = this.pageService.getGameUrl(this.gamePlayService.gamePlayStartParameters.game, 'info');
